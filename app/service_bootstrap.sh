...
start_app() {
  ...
  echo "Peer-to-peer mTLS listener on port ${DRONE_PEER_MTLS_PORT:-8543} (browser/admin ports above never request a client certificate)"
  echo "Startup log: $STARTUP_LOG"
}
...
stop_app() {
  ...
  peer_mtls_port="${DRONE_PEER_MTLS_PORT:-8543}"
  kill -9 $(lsof -t -i:"$peer_mtls_port") 2>/dev/null || true
  rm -f "$PID_FILE"
