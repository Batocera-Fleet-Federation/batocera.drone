...
# Generous on purpose: even a slow candidate (a few retries, each capped at
# TMDB_MAX_RETRY_DELAY_SECONDS) should still tick well inside this window
# under any real TMDb-availability condition; only a genuine hang (e.g. the
# uncapped-Retry-After bug this replaces) goes this long silent.
...
def _ensure_schema(connection: sqlite3.Connection) -> None:
    ...
    _ensure_column(connection, "movie_scrape_jobs", "updated_at", "TEXT")
...
_COLUMNS = (
    ...
    "failed_count, current_movie, error_message, started_at, completed_at, stop_requested, updated_at"
)
...
def create_running(settings: Any, *, rescan_all: bool, total: int) -> dict:
    ...
    with _open(settings.userdata_root) as connection:
        cursor = connection.execute(
            "INSERT INTO movie_scrape_jobs (status, rescan_all, total, started_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ...
    *,
    ...
    with _open(settings.userdata_root) as connection:
        connection.execute(
            ...
            (int(processed), str(current_movie or ""), int(matched_count), int(skipped_count), int(failed_count), _now(), job_id),
        )
...
def reconcile_if_stale(settings: Any) -> None:
    ...
    sleep (now capped, see ``TMDB_MAX_RETRY_DELAY_SECONDS`` and its
    MusicBrainz-client counterpart) left a job's background thread blocked
    for hours. ``stop_requested`` was set correctly, but the thread never
    returned to the top of its loop to see it, and there was no way to
    start a new scrape short of hand-editing the job row's status on the
    live device.
    """
    with _open(settings.userdata_root) as connection:
        row = connection.execute(
            "SELECT id, updated_at, started_at FROM movie_scrape_jobs WHERE status = ? ORDER BY id DESC LIMIT 1",
            (STATUS_RUNNING,),
        ).fetchone()
        if not row:
            return
        ...
        if age_seconds <= STALE_AFTER_SECONDS:
            return
        connection.execute(
            "UPDATE movie_scrape_jobs SET status = ?, error_message = ?, completed_at = ? WHERE id = ? AND status = ?",
            (
                STATUS_ERROR,
