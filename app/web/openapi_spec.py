...
def _schemas() -> Dict[str, Schema]:
    ...
    return {
        ...
        "AuthSessionResponse": _object(
            {"authenticated": _boolean(), "setup_required": _boolean(), "username": _string(nullable=True)},
            ("authenticated", "setup_required"),
            description="Whether the caller's session is valid and whether first-boot credential setup is still required.",
        ),
        ...
        "AuthSetupResponse": _object(
            ...
            ("status", "username"),
            description="First-boot setup completed; the session cookie is set via Set-Cookie.",
        ),
        ...
        "AdminTorrentsResponse": _object(
            {
                ...
                "vpn_required": _boolean("Whether torrent traffic is required to use the managed VPN tunnel"),
                "vpn_connected": _boolean("Whether the managed VPN is connected with a tun0 address"),
                "privacy_blocked": _boolean("Whether aria2 is stopped because the required VPN tunnel is unavailable"),
        ...
        "VpnStatusResponse": _object(
            {
                ...
                "privacy_expected": _boolean("Whether resolver access must remain fail-closed until the VPN reconnects"),
                "dns_protected": _boolean("Whether only VPN-provided/configured DNS servers are active"),
                "dns_servers": _array(_string("DNS server currently enforced for the VPN tunnel")),
                "dns_error": _string("Reason resolver protection is blocked or degraded"),
