...
The Swarm page can also reference selected systems from a paired Drone's ROM and BIOS library without copying them. New references negotiate a Drone-owned, read-only NFSv4 export over the paired mTLS API, use a same-LAN route before Tailscale when available, and fall back automatically to Batocera's standard read-only SMB share when NFS is unavailable or the source runs an older Drone release. Source-side ROM symlinks (including absolute links into external drives) are resolved into real per-system directories inside the private export. The client validates each selected system's gamelist and exposes it through one atomic, Drone-owned EmulationStation overlay; it never renames, replaces, mounts over, or creates links inside the local ROM folders. Unsafe paths, invalid XML, and gamelists with no reachable games fail safely, while isolated stale entries are reported without blocking the valid games. Gamelist-only discovery is enabled only while the reference is active, preventing EmulationStation from recursively walking large folder-based games over a latency-sensitive network mount, and the exact prior setting is restored on detach. Existing local BIOS files always win; only missing BIOS files receive read-only fallback links. Saves and other configuration remain local. Existing legacy ROM-link references are safely restored and migrated before the new layout is activated. TCP port 2049 must be reachable between the two Drones on the LAN or permitted by the applicable Tailscale ACL. Source authorizations are restricted to the paired Drone's exact known LAN/Tailscale IPv4 addresses, restored after reboot, revoked on detach/unpair, and removed by the uninstaller.
...
The database filename is retained for in-place compatibility with existing ROM caches; it now also contains keyed application state, completed action records, paired-peer state, and the ROM/BIOS/save/movie/show caches. Movies are scanned from `/userdata/movies` and Plex-organized TV libraries from `/userdata/shows` (`MOVIES_ROOT` and `SHOWS_ROOT` override them). Existing JSON caches are migrated on first use. On each poll Drone scans file size and modified time first, upserts only added or changed rows, deletes rows for removed assets, and hashes only new or changed ROM files (a sampled fingerprint, not a full-file MD5). A completed pass simply marks the local cache clean — nothing is uploaded anywhere. Discovery and hashing work are checkpointed during progress so a restarted Drone resumes from completed hashes instead of starting the metadata build over. Hash reads yield between chunks to avoid monopolizing slower storage, and other filesystem walks are deferred while metadata work is active.
