...
# The actual install/upgrade/service logic lives in the versioned app bundle
# at app/install_tailscale.sh, so it auto-updates with every Drone release
# (same reasoning as app/service_bootstrap.sh) and so the running Drone -- its
# self-update hook and the Controls page "Install / Update Tailscale" button --
# executes the exact same code path this installer does. One source of truth.
install_tailscale_mesh() {
  ...
  TS_INSTALLER="$WORK_DIR/app/install_tailscale.sh"
  if [ ! -f "$TS_INSTALLER" ]; then
    echo "Tailscale installer missing from the app bundle ($TS_INSTALLER); skipping mesh setup."
    return 1
  fi

  echo ""
  echo "Setting up the Tailscale mesh (tailnet) ..."
  # "install" here, not "ensure": a first-time installer should never silently
  # replace binaries an operator already put in place. Upgrades happen on Drone
  # self-update and from the Controls page button.
  if ! sh "$TS_INSTALLER" install; then
    return 1
  fi
  ...
  if "$TS_DIR/bin/tailscale" --socket="$TS_SOCKET" status >/dev/null 2>&1; then
    ...
    echo "Or paste an auth key into the Drone's Swarm page, or re-run this"
    echo "installer with TS_AUTHKEY=tskey-auth-... for hands-free enrollment."
  fi
  return 0
}
